...
export function createSessionFromPlan(plan, customExercises = [], now = new Date()) {
  ...
  const exercises = (plan.exercises || []).map((item, index) => {
    ...
    if (plan.workoutType === WORKOUT_TYPES.STRENGTH) return { ...common, sets: item.sets.map((set) => ({ id: createId("session-set"), plannedReps: Number(set.targetReps), plannedWeight: Number(set.targetWeight), targetRir: set.targetRir != null ? Number(set.targetRir) : null, actualReps: Number(set.targetReps), actualWeight: Number(set.targetWeight), actualRir: set.targetRir != null ? Number(set.targetRir) : null, completed: false })) };
