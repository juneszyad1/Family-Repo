...
export function createSessionFromPlan(plan, customExercises = [], now = new Date()) {
  ...
  const exercises = (plan.exercises || []).map((item, index) => {
    ...
    const common = { id: createId("session-exercise"), exerciseId: item.exerciseId, exerciseNameSnapshot: source?.name || "Gelöschte Übung", order: index + 1, supersetId: item.supersetId || null, notes: item.notes || "" };
    if (plan.workoutType === WORKOUT_TYPES.STRENGTH) return { ...common, sets: item.sets.map((set) => ({ id: createId("session-set"), setType: set.setType || "normal", plannedReps: Number(set.targetReps), plannedWeight: Number(set.targetWeight), targetRir: set.targetRir != null ? Number(set.targetRir) : null, actualReps: Number(set.targetReps), actualWeight: Number(set.targetWeight), actualRir: set.targetRir != null ? Number(set.targetRir) : null, completed: false })) };
